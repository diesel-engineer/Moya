...
extension Endpoint {
    ...
    public func urlRequest() throws -> URLRequest {
        ...
        case let .requestParameters(parameters, parameterEncoding):
            if let parameterEncoding = parameterEncoding as? URLEncoding, parameterEncoding.destination != .httpBody {
                fatalError("Only URLEncoding that `parameterEncoding` accepts is URLEncoding.httpBody. Others like `default`, `queryString` or `methodDependent` are prohibited - if you want to use them, add your parameters to `urlParameters` instead.")
            }
